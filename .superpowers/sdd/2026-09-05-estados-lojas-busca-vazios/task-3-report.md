...
- GREEN: o teste novo passou 12/12 casos, incluindo precedência, vazios por escopo, ausência independente de cada seção, sugestões/recentes, deduplicação case-insensitive, vazio e limite customizado.

## Verificação

- `pnpm --filter @keepit/cliente test -- src/lib/searchViewState.test.ts src/lib/storeDiscovery.test.ts src/hooks/useSearchProdutos.test.ts` — 18/18 testes passaram em 3 arquivos.
- `pnpm --filter @keepit/cliente test` — 289/289 testes passaram em 43 arquivos.
